...
- **Resolved 2026-09-25 — the seed does NOT reproduce across arch (T4 vs L4).** Re-rendering the two lost Kurd knob-probe tracks on a T4 (sm_75 binary `7ad69d1c…`) with byte-identical inputs (seed `20260924`, cap `6500`, `c3050_a0.5` `33e824f2…`/`7d9324bf…`, same prompts/GGUF) produced different WAVs from the L4/sm89 originals (`97028a71…`): `t0.8/Kurd` `d783a3bb…`→`37c252c2…` (192.92→204.92 s), `rp1.4/Kurd` `01e903ed…`→`ddae993e…` (215.80→187.44 s). A cross-arch "regeneration" is therefore a **new render**, and the WAV stays the only guaranteed copy. T4 substitutes: GCS `.../pron_knob_probe/t4_regen/` (separate prefix; originals untouched); the L4 MP3s stay canonical. Records: `results/pron_knob_probe/T4_REGEN.md`.

## Inference backup is a race against the ephemeral VM

- `backup_to_gcp.py --inference` targets sync with `settle=False` (only training's checkpoint folder waits), so a pass can upload a WAV mid-write; the *next* pass heals it. On 2026-09-25 the 15-min daemon's last pass was ~15:12:26Z and the VM ended before the next (~15:27), so the two renders that finished 15:12:57 / 15:17:15 were never mirrored and were lost with `/content` (`pron_knob_probe` **was** a daemon target — this was cadence + VM death, not a coverage bug). Cheap mitigation to consider for future long sweeps: have the driver fire `backup_to_gcp.py --inference --once` (incl. the sweep's `--extra`) right after the last track, rather than relying only on the interval.
