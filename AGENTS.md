...
- Before the user starts a run, confirm the sidecars are up: `pgrep -af backup_to_gcp.py` and `pgrep -af gpu_logger.py` (or freshness of `/content/logs/gcp_backup.log`). If not, give the exact start command from the doc.
...
- **OpenCode sessions** are backed up separately by `vm-continuity` (installed and loop-started by `setup.sh`, one namespace per host). If it stopped: `vm-continuity watch --interval-minutes 15`. A global reminder lives at `~/.config/opencode/AGENTS.md`.
